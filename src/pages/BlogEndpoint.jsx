...
const BlogEndpoint = () => {
  const [selectedTab, setSelectedTab] = useState(() => {
    ...
    return navItems.some((item) => item.id === target) ? target : "main";
  });
  const sortedBlogs = useMemo(() => [...blogs].sort((a, b) => b.id - a.id), []);
  ...
  const scrollToSection = (sectionId) => {
    ...
    window.history.replaceState(null, "", `#/blogs#${sectionId}`);
  ...
  const renderBlogCard = (blog) => (
    <Blogs.BlogCard key={blog.id}>
      <Blogs.BlogCardHeader>
        <Blogs.BlogCardImage label={`${blog.title} thumbnail`} />
          ...
          <Blogs.BlogsContentContainer>
            ...
            {selectedTab !== "authors" && (
              <Blogs.FeaturedSection>
                ...
                <Blogs.FeaturedCard>
                  <Blogs.FeaturedImage
                    label={`${featuredBlog.title} hero image`}
                    aspectRatio="16 / 10"
                  />
